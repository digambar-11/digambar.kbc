...
def key_val():
    ...
    for q in questions:
        ...
        clear() 
        print("*" * 45)
        print(f"   QUESTION {count} | Winning Amount : INR {total_money - 1000} ")
        print("*" * 45)
        ...
        if ans == q["A"]:
            ...
            confirm = input(f"Are you sure want to quit? You have {total_money - 1000} INR till now\nPress \"Y\" to quit or \"N\" to continue : ").upper()
            print("Oops!! You lost, Better Luck Next time!") if confirm == "Y" and count == 1 else print(f"Congratulations!! You won {total_money} INR.")   
            break 

        elif ans == "H" :
            print("\n💡 Hint : ", q["Hint"])
            make_sure = input("\nYour Answer Please : ").upper()
            if make_sure == q["A"]:
                print(f"\n!!Congratulations!! Your answer is correct and you won INR {total_money}!")
                time.sleep(2)
            elif make_sure == "Q":
                confirm = input(f"Are you sure want to quit? You have {total_money - 1000} INR till now\nPress \"Y\" to quit or \"N\" to continue : ").upper()
                print("Oops!! You lost, Better Luck Next time!") if confirm == "Y" and count == 1 else print(f"Congratulations!! You won {total_money - 1000} INR.")   
                break 
            else:
                print(f"\nOops!! Wrong answer\nThe correct answer is {q['A']}.\nYou have to quit with {total_money - 1000} INR.")
                return
